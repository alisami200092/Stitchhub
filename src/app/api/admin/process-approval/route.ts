...
export async function POST(req: Request) {
  ...
  try {
    const { logId, decision, finalText, unitPrice, totalPrice } = await req.json();
    ...
    if (decision === "approve") {
      if (unitPrice && !isNaN(parseFloat(unitPrice))) {
        updatePayload.unitPrice = parseFloat(unitPrice).toFixed(2);
      }
      if (totalPrice && !isNaN(parseFloat(totalPrice))) {
        const cleanPrice = parseFloat(totalPrice).toFixed(2);
        updatePayload.totalPrice = cleanPrice;
        updatePayload.finalQuoteAmount = cleanPrice;
      }
    }
    ...
    const targetCustomerMetadata = processedLog.metadata as { recipientEmail?: string; invoiceNumber?: string } | null;
    const customerEmail = targetCustomerMetadata?.recipientEmail;
    const invoiceNumber = targetCustomerMetadata?.invoiceNumber;

    // Sync invoice profile
    if (decision === "approve" && invoiceNumber && totalPrice && !isNaN(parseFloat(totalPrice))) {
      await db
        .update(invoices)
        .set({
    ...
    if (decision === "approve" && customerEmail) {
      try {
        ...
        if (!apiKey) {
          console.warn("BREVO_API_KEY is not defined in environment variables. Skipping email notification.");
        } else {
          ...
          console.log(`Email notification successfully dispatched via Brevo to ${customerEmail}`);
        }
      } catch (emailError) {
        console.error("Failed to send transactional email notification via Brevo:", emailError);
      }
    }
