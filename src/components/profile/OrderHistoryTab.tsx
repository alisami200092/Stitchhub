...
function statusBadge(status: string) {
  const isPaid = status === "paid" || status === "processing" || status === "shipping" || status === "delivered";
  return (
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-0.5">View your past orders, invoices, and payment statuses. Click any row to track sourcing timeline details.</p>
      </div>
      <div className="border border-zinc-800 rounded-xl overflow-hidden mt-4">
        {invoices.length === 0 ? (
          <div className="text-center py-12 text-zinc-500 italic font-mono text-xs">
          ...
          <table className="w-full text-left font-body text-xs border-collapse">
            ...
            <tbody className="divide-y divide-zinc-800/60 text-zinc-300">
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-zinc-900/30 transition-colors cursor-pointer group">
                  <td className="p-4 font-mono font-bold text-white group-hover:text-[#d4af37] transition-colors">
                    <Link href={`/profile/history/${inv.id}`} className="block">
                      {inv.invoiceNumber}
                    </Link>
                  </td>
